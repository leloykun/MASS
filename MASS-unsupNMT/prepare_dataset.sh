...
MONO_SRC_ALL=$MONO_PATH/$SRC/all.$SRC
MONO_TGT_ALL=$MONO_PATH/$TGT/all.$TGT
MONO_SRC_TRAIN=$MONO_PATH/$SRC/train.$SRC
MONO_TGT_TRAIN=$MONO_PATH/$TGT/train.$TGT
MONO_SRC_VALID=$MONO_PATH/$SRC/valid.$SRC
MONO_TGT_VALID=$MONO_PATH/$TGT/valid.$TGT
MONO_SRC_TEST=$MONO_PATH/$SRC/test.$SRC
MONO_TGT_TEST=$MONO_PATH/$TGT/test.$TGT

MONO_SRC_ALL_TOK=$MONO_SRC_ALL.tok
MONO_TGT_ALL_TOK=$MONO_TGT_ALL.tok
...
# tokenize data
if ! [[ -f "$MONO_SRC_ALL_TOK" && -f "$MONO_SRC_TRAIN_TOK" && -f "$MONO_SRC_VALID_TOK" && -f "$MONO_SRC_TEST_TOK" ]]; then
  echo "Tokenize $SRC monolingual data..."
  eval "cat $MONO_SRC_ALL   | $REPLACE_UNICODE_PUNCT | $NORM_PUNC | $REM_NON_PRINT_CHAR | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $SRC > $MONO_SRC_ALL_TOK"
...
if ! [[ -f "$MONO_TGT_ALL_TOK" && -f "$MONO_TGT_TRAIN_TOK" && -f "$MONO_TGT_VALID_TOK" && -f "$MONO_TGT_TEST_TOK" ]]; then
  echo "Tokenize $TGT monolingual data..."
  eval "cat $MONO_TGT_ALL   | $REPLACE_UNICODE_PUNCT | $NORM_PUNC | $REM_NON_PRINT_CHAR | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $TGT > $MONO_TGT_ALL_TOK"
...
echo "$SRC monolingual data tokenized in: $MONO_SRC_ALL_TOK, $MONO_SRC_TRAIN_TOK, $MONO_SRC_VALID_TOK, $MONO_SRC_TEST_TOK"
echo "$TGT monolingual data tokenized in: $MONO_TGT_ALL_TOK, $MONO_TGT_TRAIN_TOK, $MONO_TGT_VALID_TOK, $MONO_TGT_TEST_TOK"
echo "[====================================================]"

# learn BPE codes
if [ ! -f "$BPE_CODES" ]; then
  echo "Learning BPE codes..."
  $FASTBPE learnbpe $CODES $MONO_SRC_ALL_TOK $MONO_TGT_ALL_TOK > $BPE_CODES
fi
...
if ! [[ -f "$SRC_ALL_BPE" ]]; then
  ...
  $FASTBPE applybpe $SRC_ALL_BPE   $MONO_SRC_ALL_TOK   $BPE_CODES
...
if ! [[ -f "$TGT_ALL_BPE" ]]; then
  ...
  $FASTBPE applybpe $TGT_ALL_BPE   $MONO_TGT_ALL_TOK   $BPE_CODES
...
eval "cat $PARA_SRC_VALID | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $SRC > $PARA_SRC_VALID_TOK"
eval "cat $PARA_TGT_VALID | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $SRC > $PARA_TGT_VALID_TOK"
eval "cat $PARA_SRC_TEST  | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $SRC > $PARA_SRC_TEST_TOK"
eval "cat $PARA_TGT_TEST  | python3 tools/lowercase_and_remove_accent.py | tools/tokenize.sh $SRC > $PARA_TGT_TEST_TOK"
